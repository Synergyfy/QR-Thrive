...
describe('QRCodesService', () => {
    ...
    qRCode: {
      create: jest
        ...
        .mockImplementation((args) =>
          Promise.resolve({ id: 'some-id', ...args.data }),
        ),
      findMany: jest.fn().mockResolvedValue([]),
      findFirst: jest
        .fn()
        .mockResolvedValue({ id: 'existing-id', userId: 'user-pro' }),
  ...
  it('should throw ForbiddenException when FREE user trial expires', async () => {
    ...
    await expect(service.create('user-free-old', dto)).rejects.toThrow(
      ForbiddenException,
    );
  });
  ...
  it('should block scanning a QR after trial expires', async () => {
    await expect(
      service.recordScan('short-free-old', '127.0.0.1', 'ua'),

...
const MenuPreview: React.FC<MenuPreviewProps> = ({ data, onButtonClick, onSubmit, isReadOnly }) => {
  ...
  const [cart, setCart] = useState<
    Record<string, { item: any; quantity: number }>
  >({});
  const [view, setView] = useState<
    "menu" | "checkout" | "details" | "success" | "item-details"
  >("menu");
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-white">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">
          {data?.successTitle || "Order Placed!"}
        </h2>
        <p className="text-gray-500 mb-8">
          {data?.successMessage ||
            `Thank you, ${details.name}. Your order is on the way.`}
        </p>
        <div className="flex flex-col gap-3 w-full max-w-[260px]">
          <button
            ...
            className="w-full px-8 py-3 bg-blue-600 text-white font-bold rounded-2xl"
          >
            Back to Menu
          </button>
    ...
    return (
      <div className="flex-1 flex flex-col bg-gray-50 -mx-6 -mt-6 min-h-full p-6">
        <div className="flex items-center gap-2 mb-6">
          <button onClick={() => setView("checkout")} className="p-2 -ml-2">
            <ArrowLeft />
          </button>
        ...
        <div className="space-y-4">
          {(
            data?.customFields || [
              ...
              { id: "4", label: "Address", type: "textarea" },
            ]
          ).map((field) => (
            <div key={field.id} className="space-y-1">
              ...
              {field.type === "textarea" ? (
                <textarea
                  ...
                  className="w-full p-4 rounded-2xl text-sm border-0"
                  value={
                    (details as any)[
          ...
          <button
            ...
            className="w-full max-w-[calc(100%-32px)] mx-auto block py-4 bg-gray-900 text-white font-bold rounded-2xl text-center"
          >
            Confirm Order
          </button>
        </div>
  ...
  if (view === "checkout") {
    return (
      <div
        className="flex-1 flex flex-col bg-gray-50 -mx-6 -mt-6 min-h-full scrollbar-hide"
      ...
      >
        ...
        <div className="flex-1 p-6 space-y-4 overflow-y-auto scrollbar-hide">
          {Object.values(cart).map((c, i) => (
            <div
              key={i}
              className="bg-white p-4 rounded-3xl border border-gray-100 flex items-center gap-4 shadow-sm"
            >
              {c.item.image && (
                <img
                  src={c.item.image}
          ...
          <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-4 mt-6">
            ...
            <textarea
              ...
              className="w-full p-4 bg-gray-50 rounded-2xl text-sm border-0 focus:ring-0"
              rows={2}
              value={note}
  ...
  if (view === "item-details" && selectedItem) {
    const inCart = cart[selectedItem.id];
    return (
      <div className="flex-1 flex flex-col relative bg-white -mx-6 -mt-6 min-h-full">
        {/* Large Image Header */}
        <div className="relative h-[40vh] sm:h-[50vh] w-full bg-gray-50 flex items-center justify-center">
          <button
            onClick={() => setView("menu")}
            className="absolute top-6 left-6 p-3 bg-white/80 backdrop-blur-md rounded-2xl text-gray-900 shadow-lg z-10 hover:bg-white transition-all active:scale-95"
          >
        ...
        {/* Content */}
        <div className="flex-1 bg-white rounded-t-[3rem] -mt-10 relative z-10 px-8 py-10 space-y-6">
          <div className="flex justify-between items-start gap-4">
            <h2 className="text-3xl font-black text-gray-900 leading-tight">
              {selectedItem.name}
            </h2>
            <p className="text-2xl font-black text-blue-600 shrink-0">
              {data?.currency || "$"}
        ...
        <div className="p-6 bg-white border-t border-gray-50 sticky bottom-0 z-20">
          {inCart ? (
            <div className="flex items-center justify-between gap-6 bg-gray-900 p-2 rounded-[2rem] shadow-xl shadow-gray-200">
              <div className="flex items-center gap-4 pl-4">
                <button
                  onClick={() => removeFromCart(selectedItem)}
                  className="w-12 h-12 rounded-full bg-gray-800 text-white flex items-center justify-center hover:bg-gray-700 transition-colors active:scale-90"
                >
                  <X className="w-5 h-5" />
                </button>
                ...
                <button
                  ...
                  className="w-12 h-12 rounded-full bg-blue-600 text-white flex items-center justify-center hover:bg-blue-500 transition-colors active:scale-90"
                >
  ...
  return (
    <div
      className="flex-1 flex flex-col relative -mx-6 -mt-6"
    ...
    >
      ...
      <div className="flex-1 bg-white rounded-t-[32px] px-6 py-8 space-y-4">
        ...
        {onButtonClick && (data as any)?.buttonUrl?.startsWith("qr:") && (
          <button
            onClick={() =>
              onButtonClick((data as any).buttonUrl.replace("qr:", ""))
            }

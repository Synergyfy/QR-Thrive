    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
      ...
      <main className="flex-1 pt-24 sm:pt-28 pb-16 sm:pb-20 px-4">
        <div className="max-w-7xl mx-auto space-y-16 sm:space-y-24">
          ...
          {/* Call to Action */}
          <div className="bg-slate-900 rounded-3xl p-8 sm:p-12 lg:p-20 text-center text-white relative overflow-hidden flex flex-col items-center">
            ...
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black mb-4 sm:mb-6 relative z-10">Ready to transform your offline marketing?</h2>
            <p className="text-slate-300 max-w-2xl text-base sm:text-lg mb-8 sm:mb-10 relative z-10">
              Join thousands of businesses already using QR Thrive to connect the physical world with their digital experiences.
            </p>
            <a href="/" className="inline-block px-8 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-blue-500 transition-all shadow-xl shadow-blue-500/20 active:scale-95 relative z-10">
              Start Building Now
            </a>
          </div>
...
function FeatureCard({ icon: Icon, title, desc, color }: any) {
  return (
    <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 hover:shadow-xl hover:shadow-slate-200/50 hover:border-blue-100 transition-all duration-300 group cursor-default">
      <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center mb-5 transition-transform group-hover:scale-110 group-hover:rotate-3 ${color}`}>
        <Icon className="w-6 h-6 sm:w-7 sm:h-7" />
      </div>
      <h3 className="text-lg sm:text-xl font-bold text-slate-800 mb-2 sm:mb-3">{title}</h3>
      <p className="text-slate-500 leading-relaxed font-medium text-sm sm:text-base">{desc}</p>
    </div>
  );
}
